 ...
 */
	...
	this.handleResize = function () {
		...
		var box = { top: 0, bottom: 0, left: 0, width: 0, height: 0, right: 0 };

    try {

        // We only override the prototype in browsers that throw
        ...
        Element.prototype.getBoundingClientRect = function () {
            try {
                // Attempt to return a ClientRect instance
                return getBoundingClientRect.call( this );
            } catch ( e ) {
                // Return a ClientRect-ish object if necessary
...
var xforms = ticks[0][i].transform.baseVal; // An SVGTransformList
var firstXForm = xforms.getItem(0);
var scalar2 = 0;
if (firstXForm.type == SVGTransform.SVG_TRANSFORM_TRANSLATE){
      scalar2 = firstXForm.matrix.f;
}
            var valueArr = /translate\(((?:-|\d|.)+),((?:-|\d|.)+)\)/g.exec(nattr);
	        var tick_center = (new THREE.Vector3).addVectors(axis_start, nv_start2end.clone().multiplyScalar(scalar2));
	        ...
	        //generate labels
	        var text = ticks[0][i].childNodes[1].textContent;
